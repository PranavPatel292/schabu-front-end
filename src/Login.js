...
class Login extends React.Component {
    ...
    callAPI(val) {
        let urlFetch = "https://schabu-web-app-nodejs.azurewebsites.net/testAPI?firstname=" + val.name + "&email=" + val.email + "&phone=" + val.phone;
        fetch(urlFetch)
            .then(res => res.text())
            .then(res => {this.setState({ apiResponse: res }); if(this.state.apiResponse == "Successfull"){
                alert("You will recieve a email shortly for the Interview process.!")
            }
        });
